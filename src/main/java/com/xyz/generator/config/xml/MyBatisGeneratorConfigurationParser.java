...
import static com.xyz.generator.internal.util.StringUtility.isTrue;
import static com.xyz.generator.internal.util.StringUtility.stringHasValue;
...
public class MyBatisGeneratorConfigurationParser {
    ...
    private static final Logger  logger = Logger.getLogger(MyBatisGeneratorConfigurationParser.class);
    ...
    public Configuration parseConfiguration(Element rootNode)
        ...
        logger.info("解析"+rootNode.getNodeName()+"根节点开始");
        for (int i = 0; i < nodeList.getLength(); i++) {
            Node childNode = nodeList.item(i);

            if (childNode.getNodeType() == Node.ELEMENT_NODE) {
                logger.info("name=" + childNode.getNodeName());
            }
            ...
            } else if ("context".equals(childNode.getNodeName())) { //最主要的解析context元素
    ...
    private void parseTable(Context context, Node node) {
        ...
        //新加的属性
        String enableSelectByWhere = attributes.getProperty("enableSelectByWhere");

        String enableInsertSelective = attributes.getProperty("enableInsertSelective");

        String enableUpdateSelectiveByPrimaryKey = attributes.getProperty("enableUpdateSelectiveByPrimaryKey");
        ...
        //新加的属性
        if (stringHasValue(enableSelectByWhere)) {
            tc.setSelectByWhereStatementEnabled(isTrue(enableSelectByWhere));
        }
        ...
        //新加的属性
        if (stringHasValue(enableUpdateSelectiveByPrimaryKey)) {
            tc.setUpdateSelectiveByPrimaryKeyStatementEnabled(isTrue(enableUpdateSelectiveByPrimaryKey));
        }
