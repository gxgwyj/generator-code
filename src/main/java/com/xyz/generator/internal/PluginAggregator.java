...
public final class PluginAggregator implements Plugin {
    ...
    @Override
    public boolean sqlMapSelectByWhereElementGenerated(XmlElement element, IntrospectedTable introspectedTable) {
        boolean rc = true;

        for (Plugin plugin : plugins) {
            if (!plugin
                    .sqlMapSelectByWhereElementGenerated(element, introspectedTable)) {
                rc = false;
                break;
            }
        }
     ...
     */
    @Override
    public boolean clientSelectByWhereMethodGenerated(Method method, Interface interfaze,
                                               IntrospectedTable introspectedTable){

        boolean rc = true;
        for (Plugin plugin : plugins) {
            if (!plugin.clientSelectByWhereMethodGenerated(method, interfaze,
                    introspectedTable)) {
                rc = false;
                break;
            }
    ...
    @Override
    public boolean sqlMapInsertBatchElementGenerated(XmlElement element, IntrospectedTable introspectedTable) {
        boolean rc = true;

        for (Plugin plugin : plugins) {
            if (!plugin.sqlMapInsertBatchElementGenerated(element, introspectedTable)) {
                rc = false;
                break;
            }
        }
